...
func TestISO8601DurationConfig(t *testing.T) {
	defaultValue := 30 * time.Second
	mock := memory.NewConfig(nil)
	wrapper := NewDurationConfig(mock, defaultValue)
	...
	// Verify conversion from a byte array with a positive value
	mock.SetValue([]byte("PT1.2S"))
	val, err = wrapper.GetSafe(context.Background())
	require.NoError(t, err)
	assert.Equal(t, 1*time.Second+200*time.Millisecond, val)
	assert.Equal(t, 1*time.Second+200*time.Millisecond, wrapper.Get(context.Background()))

	// Verify conversion from a byte array with a negative value
	mock.SetValue([]byte("PT-1H"))
	val, err = wrapper.GetSafe(context.Background())
	require.NoError(t, err)
	assert.Equal(t, -1*time.Hour, val)
	assert.Equal(t, -1*time.Hour, wrapper.Get(context.Background()))

	// Invalid byte array value
	mock.SetValue([]byte("PT"))
	val, err = wrapper.GetSafe(context.Background())
	require.Error(t, err)
	assert.Equal(t, -1*time.Hour, val)
	assert.Equal(t, -1*time.Hour, wrapper.Get(context.Background()))
